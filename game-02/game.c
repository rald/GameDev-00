...
void saveSprite(char *name,int *canvas,int w,int h,int frames) {
	...
	fprintf(fout,"COLORREF *%s_pixels = {\n",name);
...
LRESULT CALLBACK WndProc (HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	switch(msg) {
		...
		case WM_PAINT:
			...
			drawThumbnail(backdc,thumbX,thumbY,gridWidth,gridHeight,thumbPixelSize,0,canvas,thumbGridColor,palette_colors,PALETTE_NUM_COLORS,PALETTE_TRANSPARENT_INDEX);
