...
void drawSprite(HDC hdc,int x,int y,int w,int h,int size,int frame,int *pixels,int *palette_colors,int paletteNumColors) {
	...
	for(int j=0;j<h;j++) {
		for(int i=0;i<w;i++) {
			fillRect(hdc,i*size+x,j*size+y,size,size,palette_colors[sprite_pixels[i+j*w+frame*w*h]]);
		}
	}
}
...
LRESULT CALLBACK WndProc (HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam) {
	...
	switch(iMsg) {
		...
		case WM_PAINT:
			...
			x=cos(frame/7.0)*100.0;
			y=sin(frame/11.0)*200.0;

			drawSprite(memoryDC,x+cx,y+cy,SPRITE_WIDTH,SPRITE_HEIGHT,pixel_size,frame%4,sprite_pixels,palette_colors,PALETTE_NUM_COLORS);
